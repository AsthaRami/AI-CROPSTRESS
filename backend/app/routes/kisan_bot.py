    ...
    try:
        ...
        if HAS_GENAI and api_key and api_key != "AIzaSyXXXXXX":
            try:
                ...
                system_prompt = (
                    f"You are 'Kisan AI', an expert agricultural assistant bot for farmers. "
                    f"Your core purpose is to provide proper, highly accurate, and empirically correct answers regarding crops, farming, pests, fertilizers, and agriculture. "
                    f"Always prioritize scientifically backed, verified agricultural practices. Provide clear, logical, step-by-step solutions with exact quantities if applicable. Never guess or hallucinate. "
                    f"If a question is completely unrelated to agriculture or farming, politely decline and state that you only answer crop-related queries. "
                    f"Answer queries thoughtfully and naturally in {target_lang}."
                )
                
                try:
                    # Try proactive gemini-1.5-flash for much faster responses
                    ...
                    # Fallback to flash if pro is unavailable or rate limited
                    model = genai.GenerativeModel('gemini-1.5-flash')
                    response = model.generate_content(
                        f"{system_prompt}\n\nUser Question: {message}",
                        generation_config={"temperature": 0.1, "top_p": 0.8}
        ...
        return jsonify({"key": "default", "response": "System error. Please try again later."}), 200
